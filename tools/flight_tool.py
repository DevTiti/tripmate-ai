...
DEFAULT_ORIGIN_IATA = os.getenv("DEFAULT_ORIGIN_IATA", "PRN")
...
# def _load_two_col_csv(filename, key_col, value_col, lowercase_key=True):
#     """Generic loader for a two-column CSV into a dict."""
...
#             if lowercase_key:
#                 key = key.lower()
#             result[key] = row[value_col].strip()
#     return result

# def load_country_aliases():
#     return _load_two_col_csv("country_aliases.csv", "alias", "country_code")

# def load_country_main_airport():
#     return _load_two_col_csv("country_main_airport.csv", "country_code", "iata", lowercase_key=False)

# def load_city_main_airport():
#     return _load_two_col_csv("city_main_airport.csv", "city", "iata")

# COUNTRY_ALIASES = load_country_aliases()
# COUNTRY_MAIN_AIRPORT = load_country_main_airport()
# CITY_MAIN_AIRPORT = load_city_main_airport()
...
def parse_route(query: str):
    ...
    # Fallback: find country/city mentions
     # Try mention-scanning before falling back to a one-sided "from X" match —
    # this catches natural phrasing like "Kosovo trip from Qatar" where the
    # destination has no explicit "to" attached to it.
    ...
    dest_mentions = [m for m in mentions if m != origin_mention]

    if origin_mention and dest_mentions:
        dep_iata = resolve_location_to_iata(origin_mention)
        arr_iata = resolve_location_to_iata(dest_mentions[0])
        return dep_iata, arr_iata

    # Pattern: flights from X (no other location mentioned)
    match = re.search(r"\bfrom\s+(.+?)(?:[.!?]|$)", q_lower)
...
if __name__ == "__main__":
    print(search_flights("Plan a 7 days Kosovo trip from Qatar"))
    print("\n" + "=" * 80 + "\n")
